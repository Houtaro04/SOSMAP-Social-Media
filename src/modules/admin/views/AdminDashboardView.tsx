...
import {
  ...
  TrendingUp, Clock, Eye, UserCheck, CheckCircle
} from 'lucide-react';
import { 
      ...
      <div className="adm-stat-grid">
        <div className="adm-stat-card orange">
          <div className="adm-stat-top">
            <div className="adm-stat-icon"><AlertTriangle size={22} /></div>
            <span className="adm-trend"><TrendingUp size={14} /> +5 hôm nay</span>
          </div>
        ...
        <div className="adm-stat-card blue">
          <div className="adm-stat-top">
            <div className="adm-stat-icon"><Clock size={22} /></div>
            <span className="adm-trend"><TrendingUp size={14} /> 12 ưu tiên cao</span>
          </div>
        ...
        <div className="adm-stat-card green">
          <div className="adm-stat-top">
            <div className="adm-stat-icon"><ShieldCheck size={22} /></div>
      ...
      <div className="adm-table-card">
        <div className="adm-table-header">
          <div className="adm-table-tabs">
            ...
            >
              Duyệt Tình nguyện viên {pendingVolunteers.length > 0 && <span className="tab-count">{pendingVolunteers.length}</span>}
            </button>
          </div>
        ...
        {activeTab === 'SOS' && (
          <table className="adm-table">
            ...
            <tbody>
              {sosReports.length === 0 ? (
                <tr><td colSpan={7} className="adm-empty-row">Không có dữ liệu</td></tr>
              ) : sosReports.map((r, idx) => {
                const statusCfg = STATUS_MAP[r.status] || { label: r.status, cls: 'badge-pending' };
                return (
                  <tr key={r.id}>
                    <td className="adm-td-num">{idx + 1}</td>
                    <td className="adm-td-addr">{r.address}</td>
                    <td>
                      <span className={`adm-badge ${LEVEL_MAP[r.level] || 'badge-low'}`}>
                        {r.level || '—'}
                      </span>
                    </td>
                    <td>
                    ...
                    <td>
                      <div className="adm-action-group">
                        <button className="adm-action-btn view" title="Xem chi tiết"><Eye size={14} /></button>
                        {r.status === 'PENDING' && (
                          <button 
          ...
          <table className="adm-table">
            ...
            <tbody>
              {rescueTasks.length === 0 ? (
                <tr><td colSpan={6} className="adm-empty-row">Không có dữ liệu</td></tr>
              ) : rescueTasks.map((t, idx) => {
                const statusCfg = STATUS_MAP[t.status] || { label: t.status, cls: 'badge-pending' };
                return (
                  <tr key={t.id}>
                    <td className="adm-td-num">{idx + 1}</td>
                    <td className="adm-td-id">{t.reportId?.slice(0, 8)}...</td>
                    <td>
                      <span className={`adm-badge ${statusCfg.cls}`}>
                        {statusCfg.label}
                      </span>
                    ...
                    <td>
                      <button className="adm-action-btn view"><Eye size={14} /></button>
                    </td>
                  </tr>
                );
              })}
          ...
          <table className="adm-table">
            ...
            <tbody>
              {pendingVolunteers.length === 0 ? (
                <tr><td colSpan={6} className="adm-empty-row">Không có tình nguyện viên chờ duyệt</td></tr>
              ) : pendingVolunteers.map((v, idx) => (
                <tr key={v.id}>
                  <td className="adm-td-num">{idx + 1}</td>
                  <td className="adm-td-name"><strong>{v.fullName}</strong></td>
                  <td>{v.phone || '—'}</td>
                  <td>{v.email || '—'}</td>
